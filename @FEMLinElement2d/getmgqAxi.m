function [masselec,kxxc,kyyc,kxc,kyc,gxelec,gyelec,nglec,v]=getmgqAxi(t,mele,IEN,X,Y,Z)
...
kxc=zeros(nglec,nglec);
kyc=zeros(nglec,nglec);
masselec=zeros(nglec,nglec);
for i=1:nglec
    for j=1:nglec
        for k=1:quadpoints
...
%%% GALERKIN
gxelec=zeros(nglec,nglec);
gyelec=zeros(nglec,nglec);
for i=1:nglec
    for j=1:nglec
        for k=1:quadpoints
